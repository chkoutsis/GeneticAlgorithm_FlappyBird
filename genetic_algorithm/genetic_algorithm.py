...
def bird_population(n_population_bird):
    bird_population = []
...
def reproduction(population):
    ...
    for i in range(len(population)):
        ...
        if parent_bird_1 == None:
            parent_bird_1 = population[random.randint(0, n_population_bird-1)]
        if parent_bird_2 == None:
            parent_bird_2 = population[random.randint(0, n_population_bird-1)]
...
def hidden_layer(parent_bird_1, parent_bird_2):
    ...
    for i in range(len(parent_1_weights)):
        for j in range(len(parent_1_weights[i])):
            new_weights_j = (
                parent_1_weights[i][j] + parent_2_weights[i][j]) / 2
